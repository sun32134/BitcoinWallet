...
TransactionBuilder.prototype.sign1 = function (vin, keyPair, k, redeemScript, hashType, witnessValue, witnessScript) {
  // TODO: remove keyPair.network matching in 4.0.0
  if (keyPair.network && keyPair.network !== this.network) throw new TypeError('Inconsistent network')
  if (!this.__inputs[vin]) throw new Error('No input at index: ' + vin)
  ...
  const signed = input.pubkeys.some(function (pubKey, i) {
    ...
    const signature = keyPair.sign1(signatureHash, k)
    input.signatures[i] = bscript.signature.encode(signature, hashType)
    return true
  })
